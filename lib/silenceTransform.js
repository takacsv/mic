...
function IsSilence(options) {
    ...
    if (options) {
      ...
      if (options.silenceThreshold) {
        that.silenceThreshold = options.silenceThreshold;
        delete options.silenceThreshold;
      }
    }
...
IsSilence.prototype._transform = function(chunk, encoding, callback) {
    ...
    var silenceThreshold = self.silenceThreshold || 2000;
    ...
    if(numSilenceFramesExitThresh) {
        for(i=0; i<chunk.length; i=i+2) {
            ...
            if(Math.abs(speechSample) > silenceThreshold) {
                self.emit('speech');
                if (debug) {
