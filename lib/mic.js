...
var mic = function mic(options) {
    ...
    var silenceThreshold = options.silenceThreshold || 2000;
    var fileType = options.fileType || 'raw';
    var debug = options.debug || false;
    var format, formatEndian, formatEncoding, soxSigned;
    var audioProcess = null, soxProcess = null;
    ...
    that.start = function start() {
        if(audioProcess === null) {
            ...
            if (!isMac && options.sox)
            {
              soxOpts = [ '-t', 'raw', '-b', bitwidth, '-e', soxSigned, '-c', channels, '-r', rate, '-', '-c', options.soxChannels, '-b', bitwidth, '-e', soxSigned, '-r', options.soxRate, '-t', 'wav', '-'];
              soxProcess = spawn('sox', soxOpts, { 'stdio': [ 'pipe', 'pipe', 'pipe' ] });
              soxProcess.on('exit', function(code, sig) {
                    if(code != null && sig === null) {
                        audioStream.emit('soxProcessExitComplete');
                        if(debug) console.log("soxProcess has exited with code = %d", code);
                    }
                });

              audioProcess.stdout.pipe(soxProcess.stdio[0]);
              soxProcess.stdout.pipe(audioStream);

              if (debug) {
                  soxProcess.stdio[2].pipe(infoStream);
              }
            }
            else
            {
    ...
    that.stop = function stop() {
        ...
        if(soxProcess != null) {
            ...
            soxProcess = null;
            if(debug) console.log("SoxProcess stopped");
        }
    };
